...
export default async function CertificationsSection() {
  ...
  const docs = await Promise.all(
    items.map((c) =>
      c.documentId ? getDocument(c.documentId) : Promise.resolve(null)
    )
  );

  const itemsWithDocs = items.map((c, i) => ({
    c,
    doc: docs[i],
  }));
  ...
  return (
    <section
      id="certifications"
      className="border-t border-line bg-teal-50/40 py-20 sm:py-28"
    >
                ...
                return (
                  ...
                  >
                    <p className="field-label">
                      {c.date ? formatMonthYear(c.date) : "Undated"}
                    </p>

                    <h3 className="mt-2 font-display text-lg font-semibold text-teal-900">
                      {c.title}
                    </h3>
